 ...
 *
 * Structure mirrors ProductsUniPerfect.jsx (header, stamp badge, closing
 * WhatsApp CTA, Navbar/Footer) but keeps the Superior blue identity.
 * Only one product currently, so it's shown as a single centered feature
 * row rather than a zigzag list — add more entries to `products` and this
...
export default function ProductsSuperior() {
  return (
    <>
      <Navbar />
      <div className="bg-white text-[#1208FF] overflow-x-hidden">
        {/* HEADER */}
        <section className="border-b border-[#1208FF]/10">
          <div className="max-w-6xl mx-auto px-5 sm:px-6 lg:px-10 pt-14 pb-10 sm:pt-20 sm:pb-16 lg:pt-24 lg:pb-20">
            ...
            >
              <motion.p
                variants={fadeUp}
                className="text-xs sm:text-base font-semibold tracking-[0.2em] uppercase text-[#1208FF] mb-4 sm:mb-5"
              >
              ...
              <motion.h1
                variants={fadeUp}
                className="font-bold uppercase leading-[0.95] text-[14vw] xs:text-5xl sm:text-6xl lg:text-7xl tracking-tight text-[#1208FF] break-words"
              >
                Our <span className="text-[#1208FF]">Products</span>
              </motion.h1>

              <motion.p
                variants={fadeUp}
                className="mt-6 sm:mt-7 mx-auto text-base sm:text-lg leading-relaxed text-[#1208FF]/70 max-w-xl px-2 sm:px-0"
              >
        ...
        <section>
          {products.map((p) => (
            <div key={p.name} className="border-b border-[#1208FF]/10">
              <div className="max-w-6xl mx-auto px-5 sm:px-6 lg:px-10 py-12 sm:py-20 lg:py-24 flex flex-col lg:flex-row items-center justify-center gap-10 sm:gap-12 lg:gap-20">
                ...
                >
                  <div className="max-w-[85%] xs:max-w-sm mx-auto lg:max-w-none">
                    <div className="relative">
                      <div className="relative aspect-square bg-[#1208FF]/[0.03] border border-[#1208FF]/10 overflow-hidden rounded-2xl">
                        <img
                          src={p.image}
                          alt={p.name}
                        ...
                        />
                        <div className="hidden absolute inset-0 items-center justify-center text-[#1208FF]/25 bg-white">
                          <PackageSearch className="w-10 h-10" strokeWidth={1.5} />
                        </div>
                      </div>

                      {/* OEM-quality stamp */}
                      <div className="absolute -top-3 -left-3 sm:-top-5 sm:-left-5 w-12 h-12 sm:w-16 sm:h-16 lg:w-[4.5rem] lg:h-[4.5rem] rounded-full bg-white border-2 border-dashed border-[#1208FF]/70 flex flex-col items-center justify-center text-center rotate-[-6deg] shadow-[0_2px_10px_rgba(0,0,0,0.08)]">
                        <BadgeCheck className="w-3 h-3 sm:w-4 sm:h-4 text-[#1208FF]" strokeWidth={1.75} />
                        <span className="text-[6px] sm:text-[7px] font-bold uppercase tracking-[0.15em] text-[#1208FF] mt-0.5 leading-none px-0.5">
                          {p.tag}
                        </span>
                      </div>
                    </div>
                ...
                >
                  <p className="text-xs sm:text-sm font-semibold tracking-[0.2em] uppercase text-[#1208FF] mb-3">
                    {p.tag}
                  </p>
                  <h3 className="font-bold uppercase text-2xl sm:text-3xl lg:text-4xl leading-tight mb-1 text-[#1208FF]">
                    {p.name}
                  </h3>

                  {p.subtitle && (
                    <p className="text-xs font-semibold uppercase tracking-wide text-[#1208FF]/50 mb-4">
                      {p.subtitle}
                    </p>
                  )}
        ...
        <section className="border-b border-[#1208FF]/10">
          <div className="max-w-6xl mx-auto px-5 sm:px-6 lg:px-10 py-14 sm:py-20 text-center">
            <h3 className="font-bold uppercase text-xl sm:text-3xl tracking-tight mb-4 text-[#1208FF]">
              Looking for something else?
            </h3>
            <p className="text-sm sm:text-base text-[#1208FF]/70 max-w-xl mx-auto mb-8 leading-relaxed px-2 sm:px-0">
